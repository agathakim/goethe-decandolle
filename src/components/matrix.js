...
export default class RelativeCounts extends React.Component {
  render() {
    ...
    const counts = data.reduce((acc, sent) => {
      const {colors} = sent;
    ...
    const pairToKey = pair => {
      return JSON.stringify(pair.sort());
    };

    const FILTER_BLANK_COLUMNS = false;
    const allowedColors = COLOR_PAIRS.reduce((mem, row) => {
      const rowSum = row.reduce(
        (acc, pair) => counts[pairToKey(pair)] + acc,
        0,
      );
      mem[getRowMode(row)] = FILTER_BLANK_COLUMNS ? rowSum > 0 : true;
      return mem;
    }, {});
    const reducedColorPairs = FILTER_BLANK_COLUMNS
      ...
      : COLOR_PAIRS;
    // const disallowedPairs = reducedColorPairs.reduce((acc, pair) => {
    //   acc[pairToKey(pair)] = true;
    //   return acc;
    // }, {});
        ...
        <g transform={`translate(${margin.left}, ${margin.top})`}>
          {reducedColorPairs.reduce((acc, row, idx) => {
            const updatedRow = row.map((pair, jdx) => {
              ...
              return (
                <rect
                  key={`${key}-${selectedFile}-${idx}`}
                  fill={pair[0] !== pair[1] ? colorScale(counts[key]) : 'white'}
                  stroke="black"
                  width={xScale(1)}
                  height={yScale(1)}
                  x={xScale(jdx)}
          ...
          {COLORS_FOR_LEGEND.filter(({color}) => allowedColors[color]).map(
            ({color, tag}, idx) => {
              return (
                <g
                  transform={`translate(${xScale(-0.1)}, ${yScale(idx + 0.6)})`}
